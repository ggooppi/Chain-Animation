...
enum AnimationDuration {
    case laborious, plodding, strolling, brisk, fast, noAnimation
...
final class AnimatedLabel: UILabel {
    ...
    func count(from: Float, to: Float, suffixTextSingular: String, suffixTextPlural: String, duration: AnimationDuration = .strolling) {
        startingValue = from
        destinationValue = to
        timer?.invalidate()
        timer = nil
        suffixSingler = suffixTextSingular
        suffixPlural = suffixTextPlural
    ...
    func countFromCurrent(to: Float, suffixTextSingular: String, suffixTextPlural: String, duration: AnimationDuration = .strolling) {
        count(from: currentValue, to: to, suffixTextSingular: suffixTextSingular, suffixTextPlural: suffixTextPlural, duration: duration)
    }

    func countFromZero(to: Float, suffixTextSingular: String, suffixTextPlural: String, duration: AnimationDuration = .strolling) {
        count(from: 0, to: to, suffixTextSingular: suffixTextSingular, suffixTextPlural: suffixTextPlural, duration: duration)
    }
    ...
    private func setTextValue(value: Float) {
        text = value <= 1 ?
            "\(String(format: customFormatBlock?() ?? decimalPoints.format, value)) \(suffixSingler)" :
            "\(String(format: customFormatBlock?() ?? decimalPoints.format, value)) \(suffixPlural)"
    }
